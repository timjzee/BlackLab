...
/** Represents SOLR field types. Unsure if this is needed? FieldTypes are implicitly loaded from the schema. */
public class BLFieldTypeSolr implements BLFieldType {
    ...
    static {
        ...
        tokenized.freeze();
        METADATA_TOKENIZED = new BLFieldTypeSolr(tokenized);
        ...
        untokenized.freeze();
        METADATA_UNTOKENIZED = new BLFieldTypeSolr(untokenized);
    }

    public static BLFieldType metadata(boolean tokenized) {
        return tokenized ? BLFieldTypeSolr.METADATA_TOKENIZED : BLFieldTypeSolr.METADATA_UNTOKENIZED;
    }
    ...
    private static synchronized BLFieldType getFieldType(boolean offsets, boolean forwardIndex, boolean contentStore) {
        ...
        return fieldTypeCache.computeIfAbsent(key, (__) -> {
            ...
            type.freeze();
            return new BLFieldTypeSolr(type);
        });
    }
}
